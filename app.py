...
if not st.session_state.medicines:
    ...
    for index, med in enumerate(st.session_state.medicines):
        ...
        # FIXED BUTTON STRUCTURE (Streamlit-safe)
        btn1, btn2 = col4.columns(2)

        if btn1.button("✅ Taken", key=f"taken_{index}"):
            st.session_state.medicines[index]["taken"] = True
            st.rerun()

        if btn2.button("❌ Delete", key=f"delete_{index}"):
...
if adherence_score >= 80:
    st.success("Great job! You're managing your medicines very well 🌟")
    st.image(draw_reward_image(), caption="🎉 Keep up the great work!")

    if adherence_score == 100:
        st.balloons()
else:
    st.warning("You're doing okay — try not to miss any doses 💙")
...
# Motivational Tip
# ---------------------------------
st.info("💡 Tip: Taking your medicines on time helps maintain good health and peace of mind.")
